 ...
 * Touch panel datasheet (weird chinese translation) :
 * https://wiki.pine64.org/images/5/51/CST816S%E6%95%B0%E6%8D%AE%E6%89%8B%E5%86%8CV1.1.en.pdf
 *
 * TODO : we need a complete datasheet and protocol reference!
 * */

Cst816S::Cst816S(TwiMaster &twiMaster, uint8_t twiAddress)
    : twiMaster{twiMaster}, twiAddress{twiAddress} {}
...
Cst816S::TouchInfos Cst816S::GetTouchInfo() {
  ...
  for (int i = 0; i < 1; i++) {
    ...
    // We fetch only the first touch point (the controller seems to handle only
    // one anyway...)
    info.isTouch = true;
    ...
    auto action = touchData[touchEventIndex + (touchStep * i)] >>
                  6; /* 0 = Down, 1 = Up, 2 = contact*/
    ...
    //      case Gestures::SlideDown: NRF_LOG_INFO("Gesture : Slide Down");
    //      break; case Gestures::SlideUp: NRF_LOG_INFO("Gesture : Slide Up");
    //      break; case Gestures::SlideLeft: NRF_LOG_INFO("Gesture : Slide
    //      Left"); break; case Gestures::SlideRight: NRF_LOG_INFO("Gesture :
    //      Slide Right"); break; case Gestures::SingleTap:
    //      NRF_LOG_INFO("Gesture : Single click"); break; case
    //      Gestures::DoubleTap: NRF_LOG_INFO("Gesture : Double click"); break;
    //      case Gestures::LongPress: NRF_LOG_INFO("Gesture : Long press");
    //      break; default : NRF_LOG_INFO("Unknown"); break;
    //    }
  }
