...
void SpiNorFlash::Init() {
  ...
  NRF_LOG_INFO(
      "[SPI FLASH] Manufacturer : %d, Memory type : %d, memory density : %d",
      id.manufacturer, id.type, id.density);
}
...
SpiNorFlash::Identification SpiNorFlash::ReadIdentificaion() {
  ...
  spi.Read(&cmd, 1, reinterpret_cast<uint8_t *>(&identification),
...
void SpiNorFlash::Read(uint32_t address, uint8_t *buffer, size_t size) {
  ...
  uint8_t cmd[cmdSize] = {static_cast<uint8_t>(Commands::Read),
                          (uint8_t)(address >> 16U), (uint8_t)(address >> 8U),
                          (uint8_t)address};
...
void SpiNorFlash::Write(uint32_t address, const uint8_t *buffer, size_t size) {
  ...
  while (len > 0) {
    ...
    uint8_t cmd[cmdSize] = {static_cast<uint8_t>(Commands::PageProgram),
                            (uint8_t)(addr >> 16U), (uint8_t)(addr >> 8U),
                            (uint8_t)addr};
