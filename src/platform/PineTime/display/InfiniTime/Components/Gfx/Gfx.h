...
public:
  ...
  void Init();
  void ClearScreen();
  void DrawString(uint8_t x, uint8_t y, uint16_t color, const char *text,
                  const FONT_INFO *p_font, bool wrap);
  void DrawChar(const FONT_INFO *font, uint8_t c, uint8_t *x, uint8_t y,
                uint16_t color);
  ...
  void SetScrollArea(uint16_t topFixedLines, uint16_t scrollLines,
...
private:
  ...
  struct State {
    State()
        : busy{false}, action{Action::None}, remainingIterations{0},
          currentIteration{0} {}
    volatile bool busy;
