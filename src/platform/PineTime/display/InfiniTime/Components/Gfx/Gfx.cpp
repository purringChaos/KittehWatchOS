...
void Gfx::FillRectangle(uint8_t x, uint8_t y, uint8_t w, uint8_t h,
                        uint16_t color) {
...
void Gfx::DrawString(uint8_t x, uint8_t y, uint16_t color, const char *text,
                     const FONT_INFO *p_font, bool wrap) {
  ...
  for (size_t i = 0; text[i] != '\0'; i++) {
    ...
    uint16_t char_width = text[i] == ' ' ? (p_font->height / 2)
...
void Gfx::DrawChar(const FONT_INFO *font, uint8_t c, uint8_t *x, uint8_t y,
                   uint16_t color) {
  ...
  lcd.NextDrawBuffer(reinterpret_cast<const uint8_t *>(&buffer),
                     bytes_in_line * 8 * 2);
  WaitTransfertFinished();
...
void Gfx::Sleep() { lcd.Sleep(); }

void Gfx::Wakeup() { lcd.Wakeup(); }
...
bool Gfx::GetNextBuffer(uint8_t **data, size_t &size) {
  ...
  if (state.action == Action::FillRectangle) {
    *data = reinterpret_cast<uint8_t *>(buffer);
    ...
    uint16_t bytes_in_line =
        CEIL_DIV(state.font->charInfo[char_idx].widthBits, 8);

    for (uint16_t j = 0; j < bytes_in_line; j++) {
      for (uint8_t k = 0; k < 8; k++) {
        if ((1 << (7 - k)) &
            state.font
                ->data[state.font->charInfo[char_idx].offset +
                       ((state.currentIteration + 1) * bytes_in_line) + j]) {
...
void Gfx::WaitTransfertFinished() const { ulTaskNotifyTake(pdTRUE, 500); }

void Gfx::SetScrollArea(uint16_t topFixedLines, uint16_t scrollLines,
